...
class ErrorsApiTransport extends common.Service {
    constructor() {
        super({
    ...
    async pollForNewEvents(service, time, timeout) {
        ...
        while (Date.now() < timeLimit) {
            ...
            if (!groupId) {
                ...
                groups.forEach(group => {
                    const match = group.affectedServices.find(context => context.service === service);
                    if (match) {
            ...
            const filteredEvents = events.filter(event => event.serviceContext.service === service &&
...
exports.ErrorsApiTransport = ErrorsApiTransport;
//# sourceMappingURL=errors-transport.js.map